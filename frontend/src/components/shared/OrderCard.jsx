...
export function OrderCard({ order, onClick }) {
  ...
  return (
    <Card onClick={onClick} className="overflow-hidden cursor-pointer hover:shadow-lg transition-all">
      <CardBody>
        ...
        <div className="space-y-2 mb-3">
          ...
          <p className="text-gray-700">
            Montant: <span className="font-semibold text-orange-500">
              {formatCurrency((order.total_amount || 0) + (order.delivery_fee_student || order.delivery_fee || 0))}
            </span>
          </p>
        </div>
